...
namespace ClassLib
{
...

    {
        ...
        public Output calc()
        {
            ...
            double Re = w*d/v;
            //Шахматное(Re > 1000)
            Nu_sh = 0.41 * Math.Pow(Re * Math.Pow(10, 3), 0.6) * Math.Pow(Pr, 0.36);
            //Коридорное расположение (Re > 1000)
            Nu_k = 0.22 * Math.Pow(Re * Math.Pow(10, 3), 0.65) * Math.Pow(Pr, 0.25);
            ...
            //int t_k = 100;     
            double k_l = ((0.8 + (1.6 * 0.11)) * (1 - 0.00038 * (100 + 273))) / Math.Pow((0.11 + 0.13) * d, 0.5);
            ...
            double a_dl =( c_pr * ((e_d / a_d) * Math.Pow((t + 273) / 100, 4) - Math.Pow((100 + 273) / 100, 4))) / ((t+273) - (100 +273));
            return new Output
            {
                ps = Math.Round(ps, 4),
                s = Math.Round(s,4),
